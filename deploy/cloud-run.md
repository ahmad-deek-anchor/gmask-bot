...
**2026-09-16, later:** access control shipped (revision 8, `access/`), then threaded channel answers with the `<@asker>` mention (`SLACK_REPLY_IN_THREAD=1` set with `--update-env-vars`, `SLACK_TAG_ASKER` default on).

**Redeployed 2026-09-16** as revision `trading-signals-slack-00004-jsx` (Messari news / taxonomy / ETF / Intel / signals tools, CME futures + BTC ETF on-chain tools, FRED macro tools; env `MESSARI_CACHE_PATH=/tmp/messari_assets_cache.json` added with `--update-env-vars`; entrypoint pinned with `--command python --args cloudrun_entry.py,-v`). `gm-bot` was granted `secretmanager.secretAccessor` on `messari_api_key2` the same day; the FRED key (`fred_api_key`) was created and granted to `gm-bot` later the same day (revision 5 picked it up). The snapshot job `trading-signals-snapshot` now runs `--sources signals,etf,cme`. When the gcloud user login has expired, `export CLOUDSDK_AUTH_ACCESS_TOKEN=$(gcloud auth application-default print-access-token)` lets every gcloud command here run on the ADC token.
