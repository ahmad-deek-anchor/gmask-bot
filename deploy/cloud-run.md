...
**Redeployed 2026-09-16** as revision `trading-signals-slack-00004-jsx` (Messari news / taxonomy / ETF / Intel / signals tools, CME futures + BTC ETF on-chain tools, FRED macro tools; env `MESSARI_CACHE_PATH=/tmp/messari_assets_cache.json` added with `--update-env-vars`; entrypoint pinned with `--command python --args cloudrun_entry.py,-v`). `gm-bot` was granted `secretmanager.secretAccessor` on `messari_api_key2` the same day; the FRED key (`fred_api_key`) does not exist yet, so the macro tools answer "not available" until it is created and granted. The snapshot job `trading-signals-snapshot` now runs `--sources signals,etf,cme`. When the gcloud user login has expired, `export CLOUDSDK_AUTH_ACCESS_TOKEN=$(gcloud auth application-default print-access-token)` lets every gcloud command here run on the ADC token.
